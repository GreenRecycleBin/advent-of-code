...
(defn f [coll]
  (loop [coll coll
         ...
         all-children #{}]
    (if (seq coll)
      (let [[parent weight children] (first coll)]
        (recur (rest coll)
               (assoc m parent children)
               (assoc all-weight parent weight)
               (apply conj all-children children)))

      [m all-weight all-children])))

(defn bottom [m children]
  (first (clojure.set/difference (into #{} (keys m)) children)))

(defn total-weight [disc m weight]
  (apply + (weight disc) (map #(total-weight % m weight) (m disc))))

(defn find-wrong-weight [parent diff m weight]
  (let [children (m parent)
        weights (map #(total-weight % m weight) children)
        weight->disc (zipmap weights children)

        wrong-weight (->> weights
                          frequencies
                          (filter (fn [[_ v]] (= v 1)))
                          ffirst)

        disc-with-wrong-weight (weight->disc wrong-weight)]

    (if disc-with-wrong-weight
      (if diff
        (recur disc-with-wrong-weight diff m weight)

        (let [correct-weight
              (first (clojure.set/difference (into #{} (distinct weights))
                                             #{wrong-weight}))

              diff (- correct-weight wrong-weight)]
          (recur disc-with-wrong-weight diff m weight)))

      (+ (weight parent) diff))))

(let [[m weight children]
      (-> input
          clojure.string/split-lines

          (as-> lines
            (for [line lines
                  :let [matches (re-seq #"(?i)(?:[a-z]|\d)+" line)]]
                [(first matches)
                 (Integer/parseInt (second matches))
                 (drop 2 matches)]))

          f)]
  (find-wrong-weight (bottom m children) nil m weight))
