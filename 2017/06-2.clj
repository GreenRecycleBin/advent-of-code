...
(loop [input input
  ...
  (case phase
    0 (if (m input)
        (recur input #{} 0 (inc phase))

        (let [new-input (f input)]
          (recur new-input (conj m input) (inc n) phase)))
...
(loop [tortoise (f input)
  ...
  (case phase
    ...
    1 (if (= tortoise hare)
        (recur tortoise (f tortoise) 1 (inc phase))
...
(loop [tortoise input
  ...
  (if (= tortoise hare)
    ...
    (if (= steps step-limit)
      (recur hare (f hare) 1 (.pow (biginteger step-limit) 2))
      (recur tortoise (f hare) (inc steps) step-limit))))
