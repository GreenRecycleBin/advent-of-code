...
(defn g [m [target op x source compare-op y :as coll]]
  (let [m
        (cond-> m
         ((case compare-op
            "!=" not=
            "==" =
            (resolve (symbol compare-op)))

          (m source 0) (Integer/parseInt y))

         (assoc target ((case op
                          "inc" +
                          "dec" -)

                        (m target 0) (Integer/parseInt x))))]

    (cond-> m
        (> (m target 0) (:max m Integer/MIN_VALUE))
        (assoc :max (m target)))))
...
(-> input
    ...
    (as-> lines
      (for [line lines]
        (rest
         (first
          (re-seq #"(\w+) (dec|inc) (-?\d+) if (\w+) (!=|<|<=|==|>|>=) (-?\d+)"
                  line)))))

    (as-> instructions
      (reduce #(g %1 %2) {} instructions))
