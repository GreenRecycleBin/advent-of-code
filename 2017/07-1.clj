...
(defn f [coll]
  (loop [coll coll
         ...
         all-children #{}]
    (if (seq coll)
      (let [[parent children] (first coll)]
        (recur (rest coll)
               (conj all-discs parent)
               (apply conj all-children children)))

      [all-discs all-children])))
...
(-> input
    ...
    (as-> lines
      (for [line lines
            :let [matches (re-seq #"(?i)[a-z]+" line)]]
          [(first matches) (rest matches)]))

    f

    (as-> discs-children
      (let [[discs children] discs-children]
        (first (clojure.set/difference discs children)))))
